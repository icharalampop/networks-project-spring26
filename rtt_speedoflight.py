...
def compute_inefficiency(results: dict, src_lat: float, src_lon: float) -> dict:
    for city, data in results.items():
        ...
        ratio = (median_ms / theoretical_min_ms) if (median_ms is not None and theoretical_min_ms > 0) else None
...
def make_plots(results: dict):
    ...
    ax.bar(x - width/2, measured,    width, label="Measured Median RTT (ms)", color="#457b9d")
    ax.bar(x + width/2, theoretical, width, label="Theoretical Min RTT (ms)", color="#e9c46a")
    ax.set_xticks(x)
    ...
    all_distances = sorted([valid[c]["distance_km"] for c in cities])
    theor_line    = [2 * (d / FIBER_SPEED_KM_S) * 1000 for d in all_distances]
    ax.plot(all_distances, theor_line, "k--", label="Theoretical Minimum", linewidth=1.5)
    for city in cities:
        d = valid[city]
    ...
    legend_patches.append(plt.Line2D([0], [0], color="k", linestyle="--", label="Theoretical Min"))
    ax.legend(handles=legend_patches, loc="upper left")
    ax.set_xlabel("Great-Circle Distance (km)")
    ax.set_ylabel("Measured Median RTT (ms)")
    ax.set_title("RTT vs. Distance: Measured vs. Speed-of-Light Minimum")
    plt.tight_layout()
